...
import {
  ...
  Chip, Fade, Grow, Zoom, Tooltip, Badge, CircularProgress
} from '@mui/material';
import {
...
const EduStatCard = ({ title, value, change, trend, icon, color, growth, index }) => {
  const renderIcon = typeof icon === 'string' ? IconMap[icon] : icon;
  return (
            ...
            <Box sx={{ flex: 1 }}>
              <Stack direction="row" alignItems="center" spacing={1.5} sx={{ mb: 2 }}>
                <Avatar sx={{
                  ...
                  {renderIcon && React.cloneElement(renderIcon, { sx: { fontSize: 24 } })}
                </Avatar>
                <Box>
        ...
        <CardContent sx={{ p: 3 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="flex-start" sx={{ mb: 2 }}>
            <Avatar sx={{
              ...
              {renderIcon && React.cloneElement(renderIcon, { sx: { fontSize: 28 } })}
            </Avatar>
            <Chip
              label={change}
              size="small"
...
export default function MainDash() {
  ...
  if (loading || !dashboardData) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', background: `linear-gradient(135deg, ${EduColors.lighterGreen} 0%, ${EduColors.lightGreen} 100%)` }}>
        <CircularProgress sx={{ color: EduColors.primary }} />
      </Box>
    );
  }

  const eduCurrentData = dashboardData;
